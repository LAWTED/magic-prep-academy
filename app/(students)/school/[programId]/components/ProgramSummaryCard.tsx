      ...
      {!showSummary && !summaryLoading && (
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <h3 className="font-bold text-bronze">AI Program Summary</h3>
          </div>
        ...
        <div className="p-4">
          <div className="prose prose-sm max-w-none">
            <ReactMarkdown
              components={{
                h1: ({ node, ...props }) => (
                  <h1 className="text-2xl font-bold mb-4 mt-6 text-black" {...props} />
                ),
                h2: ({ node, ...props }) => (
                  <h2 className="text-xl font-bold mb-3 mt-5 text-black" {...props} />
                ),
                h3: ({ node, ...props }) => (
                  <h3 className="text-lg font-bold mb-2 mt-4 text-black" {...props} />
                ),
                p: ({ node, ...props }) => (
                  <p className="mb-4 text-bronze" {...props} />
                ),
                ul: ({ node, ...props }) => (
                ...
                li: ({ node, ...props }) => <li className="mb-1 text-bronze" {...props} />,
                a: ({ node, ...props }) => (
                  <a className="text-skyblue hover:underline" {...props} />
                ),
                blockquote: ({ node, ...props }) => (
                  <blockquote
                    className="border-l-4 border-bronze/20 pl-4 italic my-4"
                    {...props}
                  />
                ),
                code: ({ node, ...props }) => (
                  <code
                    className="bg-gold/30 rounded px-1 py-0.5 font-mono text-sm"
                    {...props}
                  />
                ),
                pre: ({ node, ...props }) => (
                  <pre
                    className="bg-gold/30 rounded p-4 overflow-x-auto my-4 font-mono text-sm"
                    {...props}
                  />
                ),
                hr: ({ node, ...props }) => (
                  <hr className="my-6 border-t border-bronze/20" {...props} />
                ),
              }}
            >
              {programSummary}
